...
// Profil SUN-25K-SG01HP3-EU-AM2, protocole triphase haute tension V1.04.
uint16_t REG_PV1_POWER = 672;
uint16_t REG_PV2_POWER = 673;
uint16_t REG_PV3_POWER = 674;
uint16_t REG_PV4_POWER = 675; // Le modele 25K possede 4 entrees PV (2 MPPT, 2+2 strings).
uint16_t REG_PV_DAILY = 529;
uint16_t REG_BATTERY_SOC = 588;
uint16_t REG_BATTERY_VOLTAGE = 587;
uint16_t REG_BATTERY_POWER = 590;
uint16_t REG_BATTERY_TEMP = 586;
uint16_t REG_GRID_POWER = 625;
uint16_t REG_GRID_STATUS = 552;
...
uint16_t BLOCK2_START = 552;
uint16_t BLOCK2_COUNT = 124;
// Compatibilite d'API seulement : la fonction VE est verrouillee sur SG01HP3,
// car R489/R490 sont des temporisations LVRT/HVRT.
...
static uint16_t block2_base_start = 552;
static uint16_t block2_base_count = 124;
...
// Le decodeur historique applique ensuite x10 a ces deux valeurs. Un
...
static void load_custom_registers() {
  ...
  DBG.printf("PV1=%d PV2=%d PV3=%d PV4=%d\n", REG_PV1_POWER, REG_PV2_POWER, REG_PV3_POWER, REG_PV4_POWER);
...
static void decode_block2(const uint8_t *rtu) {
  ...
  main_data.pv4_power = modbus_get_u16_be(rtu, REG_PV4_POWER - offset);
...
static uint16_t scaled_unsigned_power(uint16_t raw, uint16_t scale) {
  const uint32_t value = uint32_t(raw) * scale;
...
static void update_dashboard_from_data() {
  ...
  // Sur SG01HP3, les puissances PV et batterie sont exprimees par pas de 10 W.
  dashboard_data.pv1_w = scaled_unsigned_power(main_data.pv1_power, 10);
  dashboard_data.pv2_w = scaled_unsigned_power(main_data.pv2_power, 10);
  dashboard_data.pv3_w = scaled_unsigned_power(main_data.pv3_power, 10);
  dashboard_data.pv4_w = scaled_unsigned_power(main_data.pv4_power, 10);
  dashboard_data.battery_soc = main_data.bat_soc;
  dashboard_data.battery_voltage = main_data.bat_voltage_raw * 0.1f;
  dashboard_data.battery_power = scaled_power(main_data.bat_power, 1.0f, 10.0f);
...
static void deye_probe_build_report_locked() {
  String report;
  report.reserve(8192);
  report += F("RAPPORT SONDE REGISTRES SUN-25K-SG01HP3\n");
...
String deye_probe_report_text() {
  ...
  const String report = deye_probe_report.isEmpty()
    ? String("RAPPORT SONDE REGISTRES SUN-25K-SG01HP3\nAucun instantane disponible.\n")
    : deye_probe_report;
  xSemaphoreGive(data_mutex);
  return report;
}
