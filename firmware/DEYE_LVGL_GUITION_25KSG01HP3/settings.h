...
static constexpr bool DEYE_HP3_EV_SUPPORTED = false;
...
static void settings_load_ev_registers() {
  ...
  storage.end();
  // Une ancienne configuration SG02 ne doit jamais reactiver les ecritures
  // R489/R490 sur SG01HP3 : ces adresses sont des parametres LVRT/HVRT.
  cfg_ev_registers.write_enabled = false;
}
...
static bool settings_registers_valid(const CustomRegisters &regs) {
  ...
  const uint16_t block2[] = { regs.grid_power, regs.gen_power, regs.load_power, regs.battery_temp,
    regs.battery_voltage, regs.battery_soc, regs.pv1_power, regs.pv2_power, regs.pv3_power, regs.pv4_power,
...
static bool settings_save_registers(const CustomRegisters &regs) {
  ...
  // Cle dediee au HP3 : ne pas ecraser les valeurs SG02/SG05 si les
  // deux firmwares sont testes successivement sur le meme ESP32.
  const bool ok = preferences.putBytes("hp3_regs_v2", &regs, sizeof(regs)) == sizeof(regs) &&
    preferences.getBytes("hp3_regs_v2", &verify, sizeof(verify)) == sizeof(verify) &&
...
static CustomRegisters settings_load_registers() {
  ...
  if (preferences.getBytesLength("hp3_regs_v2") == sizeof(regs) &&
      preferences.getBytes("hp3_regs_v2", &regs, sizeof(regs)) == sizeof(regs) && settings_registers_valid(regs)) {
    preferences.end();
    return regs;
  }
  preferences.end();
  // Ne jamais importer implicitement les anciennes cles individuelles SG02.
  return CustomRegisters{672,673,674,675,529,588,587,590,586,625,552,520,521,653,667,526,540,541,552,3,
...
static bool settings_save_tempo(bool enabled, bool colorblind, bool ev_charger) {
  if (!DEYE_HP3_EV_SUPPORTED) ev_charger = false;
...
static void settings_load() {
  ...
  cfg_ev_charger_enabled = DEYE_HP3_EV_SUPPORTED && preferences.getBool(SETTINGS_KEY_EV_CHARGER, false);
