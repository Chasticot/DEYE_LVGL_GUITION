...
static bool settings_save_registers(const CustomRegisters &regs) {
  ...
  const bool ok = preferences.putBytes("hp3_regs_v1", &regs, sizeof(regs)) == sizeof(regs) &&
    preferences.getBytes("hp3_regs_v1", &verify, sizeof(verify)) == sizeof(verify) &&
...
static CustomRegisters settings_load_registers() {
  ...
  if (preferences.getBytesLength("hp3_regs_v1") == sizeof(regs) &&
      preferences.getBytes("hp3_regs_v1", &regs, sizeof(regs)) == sizeof(regs) && settings_registers_valid(regs)) {
    preferences.end();
    return regs;
  }
  preferences.end();
  // Ne jamais importer implicitement les anciennes cles individuelles SG02.
  return CustomRegisters{672,673,674,675,529,588,587,590,586,625,552,520,521,653,643,526,540,541,552,
