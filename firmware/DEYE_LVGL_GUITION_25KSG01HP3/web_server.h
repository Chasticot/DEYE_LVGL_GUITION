...
static String web_dashboard_json() {
  ...
  String payload;
  payload.reserve(600);
  payload += "{\"valid\":" + String(data.valid ? "true" : "false") + ",\"pv_w\":" + String(uint32_t(data.pv1_w) + data.pv2_w + data.pv3_w + data.pv4_w);
...
static String web_config_json() {
  ...
  String payload = "{\n  \"format\": \"deye-guition-sg01hp3-25k-v1\",\n  \"firmware\": \"" + String(FIRMWARE_VERSION) + "\",\n";
...
static bool web_apply_config_json(const String &json) {
  ...
  if (!web_json_string(json, "format", format) || format != "deye-guition-sg01hp3-25k-v1" ||
...
static void web_diagnostic_page() {
  ...
  page += "</table><p><a href='/ve-probe'>Sonde registres (lecture seule)</a></p><p>Les ages sont des horodatages depuis la derniere lecture reussie. Rafraichir la page pour actualiser.</p>";
  config_web.send(200, "text/html; charset=utf-8", page);
}

// Sonde de cartographie SG01HP3. Les boutons n'envoient qu'une
// demande de lecture FC03 a SolarmanReader ; aucune ecriture Modbus n'existe
// dans ce parcours.
static void web_ve_probe_page() {
  if (!web_authorized()) return;
  String page = "<!doctype html><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><title>Sonde registres</title><style>body{font:17px system-ui;background:#101b29;color:#eef4fb;max-width:760px;margin:24px auto;padding:16px}button{padding:12px;margin:6px 6px 6px 0;background:#77d5b4;color:#102030;border:0;border-radius:7px;font-weight:bold}a{color:#77d5b4}code{background:#1e3044;padding:2px 4px}</style><h1>Sonde registres SUN-25K-SG01HP3</h1><p><a href='/diagnostic'>Diagnostic</a> · lecture seule <code>FC03</code> de R0 a R1023.</p><ol><li>Ne modifie rien, puis lancer <b>1. AVANT</b> et attendre la fin.</li><li>Sans modifier le LCD, lancer <b>2. TEMOIN</b>. Les mesures variables seront ecartees.</li><li>Modifier <em>un seul</em> reglage au LCD ou dans DeyeCloud.</li><li>Lancer <b>3. APRES</b>, puis telecharger le rapport.</li></ol><p id=status>" + web_escape(deye_probe_status_text()) + "</p><button onclick=go('/ve-probe/before')>1. Instantane AVANT</button><button onclick=go('/ve-probe/reference')>2. Instantane TEMOIN</button><button onclick=go('/ve-probe/after')>3. Instantane APRES</button><p><a href='/ve-probe/report.txt' download>Telecharger le rapport sonde (.txt)</a></p><script>const token='" + web_csrf + "';async function go(p){let r=await fetch(p,{method:'POST',headers:{'X-CSRF-Token':token}});status.textContent=await r.text();setTimeout(()=>location.reload(),2500)}if(/attente|en cours/.test(status.textContent))setTimeout(()=>location.reload(),2500)</script>";
  config_web.send(200, "text/html; charset=utf-8", page);
}
...
static void web_home() {
  ...
  String page;
  page.reserve(16000);
  page = F("<!doctype html><html lang='fr'><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Configuration SUN-25K-SG01HP3</title><style>body{font:16px system-ui;background:#101b29;color:#eef4fb;max-width:760px;margin:24px auto;padding:16px}details{background:#1e3044;margin:12px 0;padding:18px;border-radius:12px}summary{cursor:pointer;font-weight:bold}label{display:block;margin:16px 0}input,select,button,textarea{font:inherit;padding:10px;border-radius:6px;border:1px solid #7990a5;box-sizing:border-box}input:not([type=checkbox]),select,textarea{display:block;width:100%;margin-top:6px}button{background:#77d5b4;cursor:pointer}a{color:#77d5b4}#status{white-space:pre-wrap}progress{width:100%}</style><h1>Configuration SUN-25K-SG01HP3</h1><p><a href='/dashboard'>Tableau de bord</a> · <a href='/diagnostic'>Diagnostic Solarman</a></p><p>Les modifications de configuration sont appliquees apres redemarrage.</p>");
  ...
  page += "<p><b>VE desactive sur SG01HP3 :</b> R489/R490 sont des temporisations LVRT/HVRT, pas des commandes de borne.</p>";
  ...
  page += WEB_FORM_END;
  page += F("<details><summary>VE / ecritures verrouillees</summary><p>Profil SG01HP3 : aucune ecriture VE n'est autorisee. R489=LVRT2_time et R490=HVRT1_time.</p></details>");
  ...
  page += WEB_FORM_END;
  page += F("<details><summary>Mise a jour OTA</summary><p>Version installee : <b>v" FIRMWARE_VERSION "</b></p><p>Le firmware et ses versions publiees se trouvent sur <a href='https://github.com/Chasticot/DEYE_LVGL_GUITION/releases' target='_blank' rel='noopener'>la page GitHub du projet</a>.</p><button type='button' onclick='checkRelease()'>Verifier si une version plus recente existe</button><p id='release-status' role='status'></p><form action='/update'><p>Apres avoir telecharge le fichier applicatif .bin depuis la release, le selectionner ci-dessous.</p><input type='file' name='firmware' accept='.bin' required><button>Installer la mise a jour</button></form><progress id='progress' max='100' value='0'></progress></details><p id='status' role='status'></p><script>const token='");
  page += web_csrf;
  page += F("';const installedVersion='" FIRMWARE_VERSION "';function semver(v){const m=String(v).trim().match(/^v?(\\d+)\\.(\\d+)\\.(\\d+)$/);return m?m.slice(1).map(Number):null}function newer(a,b){for(let i=0;i<3;i++){if(a[i]!=b[i])return a[i]>b[i]}return false}async function checkRelease(){const s=document.getElementById('release-status');s.textContent='Verification sur GitHub...';try{const r=await fetch('https://api.github.com/repos/Chasticot/DEYE_LVGL_GUITION/releases/latest',{headers:{Accept:'application/vnd.github+json'}});if(r.status===404)throw new Error('Aucune release GitHub publiee pour le moment.');if(!r.ok)throw new Error('GitHub ne repond pas (erreur '+r.status+').');const d=await r.json(),available=semver(d.tag_name),installed=semver(installedVersion);if(!available)throw new Error('Le dernier tag '+d.tag_name+' doit respecter le format vMAJEUR.MINEUR.CORRECTIF.');s.textContent=newer(available,installed)?'Une version plus recente est disponible : '+d.tag_name+' (installee : v'+installedVersion+').' : 'Le firmware est a jour : v'+installedVersion+' (derniere release : '+d.tag_name+').' }catch(e){s.textContent='Verification impossible : '+e.message}}document.querySelectorAll('form').forEach(form=>form.addEventListener('submit',event=>{event.preventDefault();if(form.action.endsWith('/ve')&&form.querySelector('[name=write_enabled]').checked&&!confirm(\"DANGER : vous allez debloquer des ecritures vers l'onduleur. Confirmer uniquement si les registres et leurs valeurs sont verifies.\"))return;const buttons=document.querySelectorAll('button');buttons.forEach(b=>b.disabled=true);const status=document.getElementById('status');status.textContent='Envoi en cours...';const request=new XMLHttpRequest();request.open('POST',form.action);request.setRequestHeader('X-CSRF-Token',token);request.upload.onprogress=e=>{if(e.lengthComputable)document.getElementById('progress').value=e.loaded/e.total*100};request.onload=()=>{status.textContent=request.responseText;buttons.forEach(b=>b.disabled=false)};request.onerror=()=>{status.textContent='Connexion interrompue. Rechargez la page pour verifier.';buttons.forEach(b=>b.disabled=false)};request.send(new FormData(form))}));</script></html>");
  config_web.send(200, "text/html; charset=utf-8", page);
}
...
static void web_server_begin() {
  ...
  config_web.on("/ve", HTTP_POST, []() {
    if (!web_write_allowed()) return;
    web_reply(false, "Ecritures VE indisponibles sur le profil SG01HP3 : R489/R490 sont reserves au LVRT/HVRT.");
  });
