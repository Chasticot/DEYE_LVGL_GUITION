...
static lv_obj_t *ta_pv1_power, *ta_pv2_power, *ta_pv3_power, *ta_pv4_power, *ta_pv_daily;
...
static bool ui_register_blocks_valid(const CustomRegisters &regs) {
  ...
  const uint16_t block2[] = {
    ...
    regs.pv3_power, regs.pv4_power, regs.battery_power, regs.grid_status, regs.smartload
  };
...
static void ui_registers_save(lv_event_t *e) {
  ...
  READ_REGISTER(pv4_power, ta_pv4_power);
...
static void ui_registers_load_values() {
  ...
  SET_TEXT(ta_pv4_power, regs.pv4_power);
...
void ui_registers_create() {
  ...
  add_reg_line(cont, "PV1 Power", &ta_pv1_power, 672, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "PV2 Power", &ta_pv2_power, 673, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "PV3 Power", &ta_pv3_power, 674, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "PV4 Power", &ta_pv4_power, 675, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "PV Daily", &ta_pv_daily, 529, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "Battery SOC", &ta_battery_soc, 588, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "Battery Volt", &ta_battery_voltage, 587, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "Battery Power", &ta_battery_power, 590, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "Battery Temp", &ta_battery_temp, 586, nullptr, 1.0f, y); y += step;

  // Registres AVEC coefficient
  add_reg_line(cont, "Grid Power", &ta_grid_power, 625, &ta_coeff_grid_power, 0.1f, y); y += step;
  add_reg_line(cont, "AC Relay Status", &ta_grid_status, 552, nullptr, 1.0f, y); y += step;
  add_reg_line(cont, "Grid Buy", &ta_grid_buy_daily, 520, nullptr, 1.0f, y); y += step;
  ...
  lv_obj_add_event_cb(default_btn, [](lv_event_t *e) {
    ...
    lv_textarea_set_text(ta_pv1_power, "672");
    lv_textarea_set_text(ta_pv2_power, "673");
    lv_textarea_set_text(ta_pv3_power, "674");
    lv_textarea_set_text(ta_pv4_power, "675");
    lv_textarea_set_text(ta_pv_daily, "529");
    lv_textarea_set_text(ta_battery_soc, "588");
    lv_textarea_set_text(ta_battery_voltage, "587");
    lv_textarea_set_text(ta_battery_power, "590");
    lv_textarea_set_text(ta_battery_temp, "586");
    lv_textarea_set_text(ta_grid_power, "625");
    lv_textarea_set_text(ta_grid_status, "552");
    ...
    lv_textarea_set_text(ta_smartload, "552");
