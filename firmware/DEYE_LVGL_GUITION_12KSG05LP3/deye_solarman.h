...
// Profil SUN-12K-SG05LP3-EU-SM2, protocole triphase basse tension V1.04.
uint16_t REG_PV1_POWER = 672;
uint16_t REG_PV2_POWER = 673;
uint16_t REG_PV3_POWER = 674; // Registre generique ; le 12K SG05LP3 possede 2 MPPT.
...
uint16_t BLOCK2_COUNT = 123;
// Compatibilite d'API seulement : la fonction VE est verrouillee sur SG05LP3,
...
static uint16_t block2_base_count = 123;
...
// coefficient de 0,1 restitue l'unite 1 W de la cartographie SG05LP3.
...
static void deye_probe_build_report_locked() {
  String report;
  report.reserve(8192);
  report += F("RAPPORT SONDE REGISTRES SG05LP3\n");
...
String deye_probe_report_text() {
  ...
  const String report = deye_probe_report.isEmpty()
    ? String("RAPPORT SONDE REGISTRES SG05LP3\nAucun instantane disponible.\n")
    : deye_probe_report;
  xSemaphoreGive(data_mutex);
  return report;
}
