...
static bool web_apply_config_json(const String &json) {
  ...
  if (!web_json_string(json, "format", format) || format != "deye-guition-sg05lp3-v1" ||
...
static void web_home() {
  ...
  String page;
  page.reserve(16000);
  page = F("<!doctype html><html lang='fr'><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Configuration Deye SG05LP3</title><style>body{font:16px system-ui;background:#101b29;color:#eef4fb;max-width:760px;margin:24px auto;padding:16px}details{background:#1e3044;margin:12px 0;padding:18px;border-radius:12px}summary{cursor:pointer;font-weight:bold}label{display:block;margin:16px 0}input,select,button,textarea{font:inherit;padding:10px;border-radius:6px;border:1px solid #7990a5;box-sizing:border-box}input:not([type=checkbox]),select,textarea{display:block;width:100%;margin-top:6px}button{background:#77d5b4;cursor:pointer}a{color:#77d5b4}#status{white-space:pre-wrap}progress{width:100%}</style><h1>Configuration Deye SG05LP3</h1><p><a href='/dashboard'>Tableau de bord</a> · <a href='/diagnostic'>Diagnostic Solarman</a></p><p>Les modifications de configuration sont appliquees apres redemarrage.</p>");
  ...
  page += "<p><b>VE desactive sur SG05LP3 :</b> R489/R490 sont des temporisations LVRT/HVRT, pas des commandes de borne.</p>";
  ...
  page += WEB_FORM_END;
  page += F("<details><summary>VE / ecritures verrouillees</summary><p>Profil SG05LP3 : aucune ecriture VE n'est autorisee. R489=LVRT2_time et R490=HVRT1_time.</p></details>");
...
static void web_server_begin() {
  ...
  config_web.on("/ve", HTTP_POST, []() {
    if (!web_write_allowed()) return;
    web_reply(false, "Ecritures VE indisponibles sur le profil SG05LP3 : R489/R490 sont reserves au LVRT/HVRT.");
  });
