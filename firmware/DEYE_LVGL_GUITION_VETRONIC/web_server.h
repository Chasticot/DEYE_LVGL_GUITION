...
static void web_home() {
  ...
  page += web_check("ev", "Activer la page VE TRONIC", cfg_ev_charger_enabled);
  ...
  page += WEB_FORM_END;
  page += F("<p>VE TRONIC WB01 : pilotage depuis le logo voiture. Aucune ecriture VE vers Deye.</p>");
...
static void web_server_begin() {
  ...
  config_web.on("/ve", HTTP_POST, []() { if (web_write_allowed()) web_reply(false, "Edition WB01 : ecritures VE Deye desactivees."); });
