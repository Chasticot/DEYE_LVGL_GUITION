...
static bool solarman_write_register(uint16_t reg, uint16_t value, uint8_t *exception) {
  (void)reg; (void)value; (void)exception;
  return false; // WB01 edition: no inverter writes, including old saved settings.
}
...
bool deye_submit_ev_command(EvDeyeCommand command) {
  (void)command;
  return false; // Charging is exclusively controlled through the VE TRONIC API.
}
...
bool deye_ev_inverter_profile_verified() {
  return false; // No Deye EVSE in this build.
}
