...
static lv_obj_t *label_ev_cable_status = nullptr;
static lv_obj_t *ev_solar_icon = nullptr;
...
extern void ui_show_vetronic(lv_event_t *e);
...
static void ui_main_create() {
  ...
  // Assez large pour "DEMAIN : ROUGE", sans retour a la ligne.
  const lv_coord_t tomorrow_width = 160;
  ...
  lv_label_set_long_mode(label_tomorrow, LV_LABEL_LONG_CLIP);
  ...
  for (uint8_t i = 0; i < 8; ++i) {
    ...
    label_tomorrow_outline[i] = ui_main_label(
      header, "", tomorrow_width, &lv_font_montserrat_16, lv_color_black(), LV_TEXT_ALIGN_LEFT
    );
    lv_label_set_long_mode(label_tomorrow_outline[i], LV_LABEL_LONG_CLIP);
  ...
  lv_obj_add_event_cb(ev_charge_group, ui_show_vetronic, LV_EVENT_CLICKED, nullptr);
  ...
  lv_obj_add_event_cb(car_icon, ui_show_vetronic, LV_EVENT_CLICKED, nullptr);
  ...
  // Etat du cable : coche verte lorsque le vehicule est branche (ou charge),
  // croix rouge lorsque le cable est debranche ou que la mesure est absente.
  label_ev_cable_status = ui_main_label(
    ev_charge_group, LV_SYMBOL_CLOSE, 18, &lv_font_montserrat_16,
    lv_color_hex(0xEF4444), LV_TEXT_ALIGN_CENTER
  );
  lv_obj_align(label_ev_cable_status, LV_ALIGN_LEFT_MID, 35, 0);
  lv_obj_clear_flag(label_ev_cable_status, LV_OBJ_FLAG_CLICKABLE);

  // Petit soleil affiche uniquement lorsque la passerelle confirme le mode
  // solaire. Il est dessine en primitives pour ne dependre d'aucune police.
  ev_solar_icon = ui_main_make_icon_container(ev_charge_group, 17, 17);
  lv_obj_align(ev_solar_icon, LV_ALIGN_LEFT_MID, 55, 0);
  lv_obj_clear_flag(ev_solar_icon, LV_OBJ_FLAG_CLICKABLE);
  const lv_color_t sun_color = lv_color_hex(0xFACC15);
  lv_obj_t *sun_core = lv_obj_create(ev_solar_icon);
  lv_obj_set_size(sun_core, 7, 7);
  lv_obj_set_pos(sun_core, 5, 5);
  lv_obj_set_style_bg_color(sun_core, sun_color, LV_PART_MAIN);
  lv_obj_set_style_border_opa(sun_core, LV_OPA_TRANSP, LV_PART_MAIN);
  lv_obj_set_style_radius(sun_core, LV_RADIUS_CIRCLE, LV_PART_MAIN);
  lv_obj_clear_flag(sun_core, LV_OBJ_FLAG_CLICKABLE);
  const lv_coord_t sun_ray_x[] = {7, 7, 0, 14, 2, 12, 2, 12};
  const lv_coord_t sun_ray_y[] = {0, 14, 7, 7, 2, 2, 12, 12};
  const lv_coord_t sun_ray_w[] = {3, 3, 3, 3, 2, 2, 2, 2};
  const lv_coord_t sun_ray_h[] = {3, 3, 3, 3, 2, 2, 2, 2};
  for (uint8_t i = 0; i < 8; ++i) {
    lv_obj_t *ray = lv_obj_create(ev_solar_icon);
    lv_obj_set_size(ray, sun_ray_w[i], sun_ray_h[i]);
    lv_obj_set_pos(ray, sun_ray_x[i], sun_ray_y[i]);
    lv_obj_set_style_bg_color(ray, sun_color, LV_PART_MAIN);
    lv_obj_set_style_border_opa(ray, LV_OPA_TRANSP, LV_PART_MAIN);
    lv_obj_set_style_radius(ray, LV_RADIUS_CIRCLE, LV_PART_MAIN);
    lv_obj_clear_flag(ray, LV_OBJ_FLAG_CLICKABLE);
  }
  lv_obj_add_flag(ev_solar_icon, LV_OBJ_FLAG_HIDDEN);
...
static void ui_main_update() {
  ...
  if (cfg_ev_charger_enabled) {
    VtSnapshot ev = vt_snapshot();
    const bool cable_connected = ev.measured && ev.state >= 1;
    lv_label_set_text(label_ev_cable_status, cable_connected ? LV_SYMBOL_OK : LV_SYMBOL_CLOSE);
    lv_obj_set_style_text_color(
      label_ev_cable_status,
      cable_connected ? lv_color_hex(0x22C55E) : lv_color_hex(0xEF4444),
      LV_PART_MAIN
    );
    if (ev.online && ev.mode == VT_SOLAR) {
      lv_obj_clear_flag(ev_solar_icon, LV_OBJ_FLAG_HIDDEN);
    } else {
      lv_obj_add_flag(ev_solar_icon, LV_OBJ_FLAG_HIDDEN);
    }
    if (ev.measured) snprintf(text, sizeof(text), "%u W", vt_power_w(ev.amps));
